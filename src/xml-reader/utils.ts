 ...
 */
export function eatSection(scanner: Scanner, open: number[], close: number[], allowUnclosed: boolean = false): SectionNode {
    const start = scanner.pos;
    ...
    if (eatArray(scanner, open)) {
        ...
        while (!scanner.eof()) {
            if (eatArray(scanner, close)) {
                return scanner.addLoc(new SectionNode(scanner.current()));
            }
